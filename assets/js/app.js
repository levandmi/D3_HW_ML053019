...
function updateToolTip(chosenXAxis, circlesGroup) {
  ...
  var toolTip = d3.tip()
    ...
    .offset([100, 0])
    .html(function(d) {
      return (`${d.state}<br>${label} ${dollar}${d[chosenXAxis]}${ending}`);
    });
...
d3.csv("./assets/data/data.csv").then(function(census_data) {
  ...
  var yLinearScale = d3.scaleLinear()
    .domain([d3.min(census_data, d => d.obesity) * .95, d3.max(census_data, d => d.obesity)*1.05])
    .range([height, 0]);
...
// =====================Attempts to make the text group at the same level as circle======================
